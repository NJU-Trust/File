...
| **投资**     |       |             |        |       |                |                 |               |
| 投资项目概要信息显示 | 孟华    | 否           | 否      | 廖均达   |                |                 |               |
| 查看项目产品概要   | 林宇超   | 是（图片路径)     | 是      | 廖均达   | 是              | 是               | 是             |
| 借款人信息      | 唐佳未   | （需要相关模块先完成） |        | 廖均达   |                |                 |               |
| 投标记录       | 唐佳未   | 否           | 否      | 廖均达   |                |                 |               |
| **账户总览**   |       |             |        |       |                |                 |               |
...
| **财务状况**   |       |             |        |       |                |                 |               |
| 数值分析       | 欧阳鸿荣  | 否           | 否      | 许杨    | 否              | 否               | 否             |
| 趋势分析       | 欧阳鸿荣  | 否           | 否      | 许杨    | 否              | 否               | 是             |
| 比例分析       | 欧阳鸿荣  | 否           | 否      | 许杨    | 否              | 否               | 部分有           |
| **财务建议**   |       |             |        |       |                |                 |               |
...
return{
    ...
    //下面有俩有点不知道怎么描述，到时我和商业组研究下，这里的GET方法得到的数据量也有点大，在考虑要不要分开
...
```json
params{
  "alipayDataPic":"picPath1"//string支付宝消费数据
  "bankDataPic":"picPath2"//string银行卡消费数据
}
return{
  submitsuccess:true//boolean
}
...
### 信息发布
    ...
    param{ 
    	...
    	"type": 'str',					//消息类别（分为失物招领和寻物启事）
    	"name":'str',  					//物品名称
    	“itemtype”:'str',				//消息类别
    	“phone”:"str",					//联系方式
    	"pic":"pic_file_url",			//对应图片 	
    	"dec":'str',					//消息描述 
    } 
...
### 消息大厅
    ...
    return{
    	...
    	"mestype": 'str',				//消息类别（分为失物招领和寻物启事）
    	“itemtype”:'str',				//消息类别
    	"name":'str',  					//物品名称
    	“phone”:"str",					//联系方式
    	"dec":'str',					//消息描述  ,   
    	"pic":"file_url",				//对应图片 	
    	"time":"2018/06/04 09:45:14",  	//发布时间
    	“headpic”:"pic_file_url",		//发布消息的用户头像
    	“username”:"username",			//发布消息的用户名
    	“loc”:"str",					//物品相关地点
    }									
